...
export class EditorCropTab {
  ...
  constructor(canvaser: Canvaser) {
    ...
    // @ts-ignore
    const section  = createNamedSection('Aspect Ratio')
  ...
  private getCropRuler() {
    ...
    rotate.onclick = () => {
      const numOfPies = Math.round((this.canvaser.crop.getAngle()-Math.PI/2)/(Math.PI/2));
      let nextAngle = (numOfPies*(Math.PI/2));
      if(nextAngle < -Math.PI) nextAngle+=2*Math.PI;

      const croppedAngle = ((nextAngle)%(2*Math.PI));
